...
import { parseL5X, type ParsedResult, type Routine, type ProgramTag } from "@/utils/parser";
...
export default function Home() {
  ...
  const handleRoutineClick = (routine: Routine, index: number) => {
  ...
  const filteredRoutines = useMemo(() => {
    ...
    if (!routineSearch.trim()) return parsedData.routines;
    
    const search = routineSearch.toLowerCase();
    return parsedData.routines.filter(
      (r) =>
        r.name.toLowerCase().includes(search) ||
        r.program.toLowerCase().includes(search)
    );
  }, [parsedData, routineSearch]);
  ...
  const filteredTags = useMemo(() => {
    if (!parsedData) return { controller: [], program: [] };
    
    const search = tagSearch.toLowerCase();
    
    const controller = search
      ? parsedData.controllerTags.filter((tag) => tag.toLowerCase().includes(search))
      : parsedData.controllerTags;
    
    const program = search
      ? parsedData.programTags.filter(
          (tag) =>
            tag.name.toLowerCase().includes(search) ||
            tag.program.toLowerCase().includes(search)
        )
      : parsedData.programTags;
              ...
              {parsedData && (
                <div className="mt-3 flex flex-wrap gap-2" data-testid="status-stats">
                  <Badge variant="secondary" className="text-xs">
                    {parsedData.routines.length} Routines
                  </Badge>
                  <Badge variant="secondary" className="text-xs">
                    {parsedData.controllerTags.length + parsedData.programTags.length} Tags
                  </Badge>
                </div>
              )}
            </Card>
              ...
              <Card className="flex-1 flex flex-col overflow-hidden">
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <Badge variant="outline" className="text-xs" data-testid="badge-tag-count">
                      {filteredTags.controller.length + filteredTags.program.length}
                    </Badge>
                  </div>
                  {tagsExpanded ? (
                    <ChevronUp className="w-5 h-5 text-muted-foreground" />
                ...
                >
                  ...
                  <div className="flex-1 overflow-y-auto p-2" role="list">
                    {filteredTags.controller.length === 0 && filteredTags.program.length === 0 ? (
                      <div className="flex flex-col items-center justify-center h-full text-muted-foreground p-6">
                        <FileText className="w-12 h-12 mb-2 opacity-50" />
                      ...
                      <div className="space-y-4">
                        {/* Controller Tags */}
                        {filteredTags.controller.length > 0 && (
                          <div>
                            <div className="flex items-center gap-2 mb-2 px-2">
                              <h3 className="text-sm font-semibold text-foreground">Controller Tags</h3>
                              <Badge variant="secondary" className="text-xs">
                                {filteredTags.controller.length}
                              </Badge>
                            </div>
                            <div className="space-y-1">
                              {filteredTags.controller.map((tag, index) => (
                                <div
                        ...
                        {/* Program Tags */}
                        {filteredTags.program.length > 0 && (
                          <div>
                            <div className="flex items-center gap-2 mb-2 px-2">
                              <h3 className="text-sm font-semibold text-foreground">Program Tags</h3>
                              <Badge variant="secondary" className="text-xs">
                                {filteredTags.program.length}
                              </Badge>
                            </div>
                            <div className="space-y-1">
                              {filteredTags.program.map((tag, index) => (
                                <div
                                  key={`program-${tag.program}-${tag.name}-${index}`}
                                  className="px-3 py-2 rounded-md hover-elevate"
                                  data-testid={`text-program-tag-${index}`}
                                  role="listitem"
                                >
                                  <div className="text-sm text-foreground">{tag.name}</div>
                                  <div className="text-xs text-muted-foreground">({tag.program})</div>
                                </div>
                              ))}
                            </div>
                          </div>
